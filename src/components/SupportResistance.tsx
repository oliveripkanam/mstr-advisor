import { fetchBtcTicker, fetchBtcKlinesNormalized } from "../lib/crypto";
...
export function SupportResistance({ onLevelHover, onTargetClick }: SupportResistanceProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        const mark = await fetchBtcTicker();
        if (!cancelled && isFinite(mark)) setCurrentPrice(mark);
        const ks = await fetchBtcKlinesNormalized('5m', 1000);
        if (!cancelled && Array.isArray(ks)) setKlines(ks as any);
      } catch {
  ...
  <Card className="p-4 overflow-hidden">
      ...
      <div className="only-mobile mb-4 space-y-2">
        ...
        <div className="flex items-center gap-2">
          <Select value={method} onValueChange={(v) => setMethod((v as any))}>
            <SelectTrigger className="h-8 w-auto px-3 truncate">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      {/* Desktop/Tablet: single row; controls at far right on PC (lg+) */}
      <div className="only-desktop items-center gap-2 mb-4 w-full pc-row">
        ...
        <div className="flex items-center gap-2 whitespace-nowrap pc-ml-auto">
          <Select value={method} onValueChange={(v) => setMethod((v as any))}>
            <SelectTrigger className="h-8 w-auto px-3 truncate">
              <SelectValue />
            </SelectTrigger>
