// Netlify Function: Binance proxy (futures + spot)
// Routes: /proxy/binance-fapi/* -> /.netlify/functions/binance/* (via netlify.toml)
// Supports:
//   - /fapi/... and /futures/... -> https://fapi.binance.com
...
function getSplatPath(eventPath: string): string {
  const prefix = '/.netlify/functions/binance';
...
export const handler = async (event: Event): Promise<Result> => {
  ...
  try {
    ...
    // Determine upstream host: futures vs spot
    const isFutures = suffix.startsWith('/fapi/') || suffix.startsWith('/futures/');
    const isSpot = suffix.startsWith('/api/');
    const base = isSpot ? 'https://api.binance.com' : 'https://fapi.binance.com';
    const upstreamUrl = `${base}${suffix}${buildQuery(event)}`;
