// Netlify Function: OKX proxy
// Routes: /proxy/okx/* -> /.netlify/functions/okx/* (via netlify.toml)
// Forwards to https://www.okx.com/:splat with friendly headers and CORS.
...
function getSplatPath(eventPath: string): string {
  const prefix = '/.netlify/functions/okx';
...
export const handler = async (event: Event): Promise<Result> => {
  ...
  try {
    ...
    const upstreamUrl = `https://www.okx.com${suffix}${buildQuery(event)}`;
    ...
    // If blocked by WAF (403), try a simple fallback via corsproxy
    if (upstreamRes.status === 403) {
      const altUrl = `https://corsproxy.io/?${encodeURIComponent(upstreamUrl)}`;
      upstreamRes = await fetch(altUrl, { method: 'GET' });
    }
